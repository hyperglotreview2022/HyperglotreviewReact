    ...
    <div>
      ...
      <div className={styles.topcontainer}>
        ...
        <div className={styles.topinfo} key={array[0]._id}>
          <Link className={styles.link} href={`/SinglePage/${array[0]._id}`}><h3>{array[0].title}</h3></Link>
      ...
      return(
          <div key={data._id} className={styles.listcontainer}>
            ...
            <div className={styles.listinfo}>
              <Link className={styles.link} href={`/SinglePage/${data._id}`}><h3>{data.title}</h3></Link>
              <p>{data.desc[0].slice(0,150)}...</p>
              <Link className={styles.link} href={`/SinglePage/${data._id}`}><button className={styles.button}>Read More</button></Link>
            </div>
          </div>
        )
    })}
